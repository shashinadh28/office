...
export default function FooterSection() {
    return (
        <footer className="border-b bg-white pt-10 sm:pt-14 md:pt-20 dark:bg-transparent">
            <div className="mb-6 sm:mb-8 md:mb-12 border-b">
                <div className="mx-auto flex max-w-5xl flex-wrap items-end justify-between gap-4 sm:gap-6 px-4 sm:px-6 pb-6">
            ...
            <div className="mx-auto max-w-5xl px-4 sm:px-6">
                <div className="grid gap-8 sm:gap-10 md:gap-12 md:grid-cols-5 md:gap-0 lg:grid-cols-4">
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:col-span-5 md:row-start-1 lg:col-span-3 gap-6">
                        {links.map((link, index) => (
                            <div key={index} className="space-y-3 sm:space-y-4 text-sm">
                                <span className="block font-medium text-sm sm:text-base">{link.group}</span>
                                {link.items.map((item, index) => (
                                    <Link key={index} href={item.href} className="text-muted-foreground hover:text-primary block duration-150 text-xs sm:text-sm">
                    ...
                    <form className="row-start-1 border-b pb-6 sm:pb-8 text-sm md:col-span-2 md:border-none lg:col-span-1">
                        <div className="space-y-3 sm:space-y-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                ...
                                <Button size="sm" className="mt-2 sm:mt-0">Submit</Button>
                            </div>
                            <span className="text-muted-foreground block text-xs sm:text-sm">Don&apos;t miss any update!</span>
                        </div>
                    </form>
                </div>
                <div className="mt-8 sm:mt-10 md:mt-12 flex flex-col sm:flex-row flex-wrap items-center sm:items-end justify-between gap-4 sm:gap-6 border-t py-4 sm:py-6">
                    <small className="text-muted-foreground order-last sm:order-first block text-center text-xs sm:text-sm">© {new Date().getFullYear()} Tailus UI, All rights reserved</small>
                    <form action="">
                        <div className="relative w-[200px]">
                            <ChevronsUpDown className="pointer-events-none absolute inset-y-0 right-2 my-auto opacity-75" size="0.75rem" />
                            <select
                                className={cn(
                                    'border-input file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground shadow-xs flex h-9 w-full min-w-32 appearance-none rounded-md border bg-transparent px-3 py-1 text-sm outline-none transition-[color,box-shadow] file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
