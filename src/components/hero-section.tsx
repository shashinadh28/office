      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div 
          className={`absolute top-4 left-4 sm:top-6 sm:left-6 md:top-1 md:left-12 transition-all duration-1000 ease-out ${
            isLoaded ? 'translate-y-0 opacity-100 blur-0' : '-translate-y-10 opacity-0 blur-md'
          }`}
        >
          <Image 
            ...
            <div className="overflow-hidden mb-6 sm:mb-8 md:mb-12">
              <p 
                className={`font-montserrat text-base sm:text-lg md:text-xl max-w-2xl transition-all duration-1500 ease-out ${
                  textAnimated ? 'translate-y-0 opacity-100 blur-0' : 'translate-y-20 opacity-0 blur-sm'
                }`}
                style={{ 
                  transitionDelay: '300ms',
                  clipPath: textAnimated ? 'inset(0 0 0 0)' : 'inset(0 100% 0 0)'
                }}
              >
        ...
        <div 
          className={`absolute bottom-6 sm:bottom-8 md:bottom-10 left-0 right-0 flex justify-center transition-all duration-800 ${
            textAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}
        ...
        >
          ...
          >
            <span className="font-montserrat text-xs sm:text-sm tracking-widest mb-2 sm:mb-3 group-hover:text-teal-500 transition-colors">SCROLL DOWN</span>
